...
newdataagg <- read.csv("newdata3.csv")
newdataagg <- newdataagg %>% select(-c('tempC', 'mintempC', 'sunHour', 'moon_illumination', 'moonrise', 'moonset', 'sunrise', 'sunset',
                                 'DewPointC', 'pressure', 'FeelsLikeC', 'winddirDegree', 'location'))

#adding the relevant columns
newdataagg$date <- as.POSIXct(newdataagg$date_time, format = "%d/%m/%Y %H:%M", tz = "UTC")
newdataagg$timeofday <- as.factor(nightday(newdataagg$date))
newdataagg$weekday <- as.factor(ifelse(isWeekday(newdataagg$date), "Weekday", "Weekend"))
newdataagg$holiday <- 0

newdataagg <- as.vector(newdataagg)

#turning it into a tsibble and getting rid of date_time since we have date
newdataagg.ts <- as_tsibble(newdataagg, index = date, key = name)
newdataagg.ts <- newdataagg.ts %>% select(-date_time, location)

fcast2 <- bikes.lm.agg %>% forecast(new_data = newdata.ts)
nyoncast2 <- fcast2 %>% filter(name == 'Nyon, Changins')
par(mfrow = c(1,2))
bikes.hts %>% filter(name == 'Nyon, Changins') %>%
  autoplot(Total) +
  autolayer(nyoncast2)
